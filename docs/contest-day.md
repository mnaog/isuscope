...
- [ ] 会話を紐付ける場合は、事前に信頼済みの共通hookと新しいCodex／Claude Codeセッションを用意し、`[context.agent]`を有効化する
...
- 会話をrunへ残す場合は、Codex・Claude Codeの共通hookを導入・信頼して新しいセッションで動作確認する
...
- 匿名viewer単位の行動遷移
- 得点の源になる値（`score-probe`、既定ではコメントアウト）

`survey-run`では、負荷構造と合わせて**得点が何でできているか**を確定します。手順は次のとおりです。

1. 当日マニュアルの得点計算を読み、項を書き出す（エンドポイントごとの重み、値の合計、減点）。
2. 重みで決まる型なら、access logのroute別成功数から模型scoreを出し、実scoreと比べる。差が数%以内なら、以後は「どのrouteを速くすると何点増えるか」で優先順位を決められる。
3. 値の合計で決まる型（ISUCON13のtip合計など）なら、`score-probe`を有効にして自分のDBやアプリからその値を読み、`score.`で始まるmetricにする。access logには出ない種類の得点である。
4. どちらでも差が大きいときは、速さが次の負荷を呼ぶ型を疑う。模型では説明できないので、1か所だけ遅らせてscoreの変化を見る実験に切り替える。

ここを飛ばすと、得点にならない経路を速くする作業に時間を使う。
